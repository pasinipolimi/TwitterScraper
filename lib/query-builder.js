...
function convertLocationToQuery( location ) {
  var radius = location.radius / 1000;
  var latitude = location.latitude;
  var longitude = location.longitude;

  return 'geocode:'+latitude+','+longitude+','+radius+'km';
}
function composeQuery( data ) {
  ...
  if( data.hash || data.hashtags ) {
    filters.push( convertHashToQuery( data.hash || data.hashtags ) );
  }
  if( data.lang ) {
  ...
  if( data.geo || data.location ) {
    filters.push( convertLocationToQuery( data.geo || data.location ) );
  }
